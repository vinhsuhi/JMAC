...
class JMAC_MODEL(BaseModel):
    def __init__(self, num_ent, num_rel, ent_info_att, args=None):
        ...
        self.completion_dropout = nn.Dropout(args.completion_dropout_rate) # 0.4
        self.atv_mlp    = nn.LeakyReLU(args.leaky_relu_w)
        ...
        self.conv1_align = RelationalAwareLayer(self.dim, self.dim, num_rel, rel_dim=self.dim, act=self.act, args=args)
        self.conv2_align = RelationalAwareLayer(self.dim, self.dim, num_rel, rel_dim=self.dim, act=self.act, args=args)
        self.conv1_completion = RelationalAwareLayer(self.dim, self.dim, num_rel, rel_dim=self.dim, act=self.act, args=args)
    ...
    def completion_loss(self, data, edge_index, edge_type, feeddict):
        ...
        for layer in range(self.args.num_gcn_layer):
            ...
            h = F.normalize(ent_completion_output[batch_h], 2, -1)
            t = F.normalize(ent_completion_output[batch_t], 2, -1)
            r = F.normalize(rel_output[batch_r], 2, -1)
