...
from modules.finding.similarity import sim
from modules.utils.util import task_divide, merge_dic
from tqdm import tqdm


def greedy_alignment(embed1, embed2, top_k, nums_threads, metric, normalize, csls_k, accurate, simi_matrix=None, return_simi=False, logger=None):
    """
...
def stable_alignment(embed1, embed2, metric, normalize, csls_k, nums_threads, cut=100, sim_mat=None):
    ...
    logger.info("generating candidate lists costs time {:.3f} s ".format(time.time() - t))
    t = time.time()
    ...
    for i, j in matching.items():
        if int(i.split('_')[-1]) == int(j.split('_')[-1]):
            n += 1
    cost = time.time() - t
    logger.info("stable alignment precision = {:.3f}%, time = {:.3f} s ".format(n / len(matching) * 100, cost))
...
# def fast_match(index, sim_mat, total_num):
#     for i in range(index):
#         max_val = np.argmax(sim_mat[i])
